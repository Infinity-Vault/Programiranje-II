...
public:
	...
	void SortirajRastuci(SortirajPo algoritam)
	{
		...
		auto sort2 = [&algoritam](T2 prvi, T2 drugi)
		{
			if (algoritam)
				return prvi > drugi;
			return prvi < drugi;
...
{
	return d1.getDan() == d2.getDan() && d1.getMjesec() == d2.getMjesec() && d1.getGodina() == d2.getGodina() && d1.getSat() == d2.getSat() && d1.getMinuti() == d2.getMinuti();
}
bool operator != (const DatumVrijeme& d1, const DatumVrijeme& d2)
{
	return !(d1 == d2);
...
public:
	...
	Uspjeh(eRazred razred, Predmet& predmet, DatumVrijeme& date)
	{
		_razred = new eRazred(razred);
	...
	{
		_razred = new eRazred(*obj.GetERazred());
	}
...
bool operator == (const Uspjeh& u1, const Uspjeh& u2)
{
	if (*u1.GetERazred() != *u2.GetERazred())
		return false;
	if (const_cast<Uspjeh&>(u1).GetPredmeti()->getTrenutno() != const_cast<Uspjeh&>(u2).GetPredmeti()->getTrenutno())
		return false;
	for (int i = 0; i < const_cast<Uspjeh&>(u1).GetPredmeti()->getTrenutno(); i++)
	{
		if (const_cast<Uspjeh&>(u1).GetPredmeti()->getElement1(i) != const_cast<Uspjeh&>(u2).GetPredmeti()->getElement1(i) || const_cast<Uspjeh&>(u1).GetPredmeti()->getElement2(i) != const_cast<Uspjeh&>(u2).GetPredmeti()->getElement2(i))
			return false;
	}
	return true;
}
...
public:
	...
	int BrojPonavljanjaRijeci(string  key)
	{
		regex pattern(key);
	...
	vector<Predmet> operator () (DatumVrijeme from, DatumVrijeme to)
	{
