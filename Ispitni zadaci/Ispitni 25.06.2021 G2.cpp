...
public:
    ...
    friend ostream& operator<<(ostream& COUT, const Tehnika& t1)
    {
        ...
        COUT << "Prosjecna ocjena za tehniku iznosi: " << t1.prosjecnaOcjenaZaTehniku() << endl;
        return COUT;
    }
...
public:
    ...
    float uspjehProsjekPolozeneTehnike()const
    {
        float prosjek = 0.0f;
        for (int i = 0; i < _polozeneTehnike.getTrenutno(); i++)
        {
            prosjek += _polozeneTehnike.getElement1(i)->prosjecnaOcjenaZaTehniku();
        }
        return prosjek / _polozeneTehnike.getTrenutno();
    }
    void DodajTehniku(Tehnika nazivTehnike, const char* napomena)
    {
...
class KaratePolaznik {
    ...
    void saljiMail(Pojas pojas, Tehnika nazivTehnike, float prosjekOcjena, float ukupniUspjeh)
    {
...
public:
    ...
    /*
     * OVDE U FUNKCIJI ADDTEHNIKU JEDINO IMAM PROBLEM SA THREDOVIMA OPET IZGLEDA ZOVEM NESTO VISKA
     * I NEZZ DALI I  //dodavanje tehnika za vise pojaseve ako ne postoji najmanje jedna tehnika za nizi pojas
     *    (ne mozemo dodati tehniku za NARANDZASTI ako ne postoji niti jedna tehnika za ZUTI pojas) iMPLEMENTIRATI NA KOJI NACIN JEDAN OD POKUSAJA MI NIJE USPIO
     *    I DOLE IMAM PROBLEM SA OPERATOROM KOD KOLEKCIJA<TEHNIKA,FLOAT>
     */
    bool AddTehniku(Pojas pojas, Tehnika nazivTehnike, string napomena)
    {
        /*
        ...
        */
        ...
        // - dodavanje istih (moraju biti identicne vrijednosti svih atributa) tehnika na nivou jednog pojasa,
        auto isIsti = [this, pojas, nazivTehnike]
        {
            for (auto& polaganje : _polozeniPojasevi)
            {
                if (polaganje.GetPojas() == pojas)
                {
                    ...
                    {
                        if (*polaganje.GetTehnike().getElement1(i) == nazivTehnike)
                        {
                            return false;
                        }
                    }
        ...
        // - dodavanje tehnika kod kojih je prosjecna ocjena manja od 3.5
        auto dodajTehniku = [this, nazivTehnike]
        {
            if (nazivTehnike.prosjecnaOcjenaZaTehniku() < 3.5f)
            {
                return false;
            }
            return true;
        ...
        //dodavanje tehnika za vise pojaseve ako ne postoji najmanje jedna tehnika za nizi pojas (ne mozemo dodati tehniku za NARANDZASTI ako ne postoji niti jedna tehnika za ZUTI pojas)
        auto prethodniPojas = [this, pojas, nazivTehnike]
        {
            if (nazivTehnike.prosjecnaOcjenaZaTehniku() < 3.5f)
            {
                return false;
            ...
            for (auto& polozeni : _polozeniPojasevi)
            {
                ...
                {
                    if (polozeni.GetTehnike().getTrenutno() < 1) { return false; }
        ...
        //provjera
        if (!isIsti() || !dodajTehniku() || !prethodniPojas())
        {
            return false;
        }
        for (auto& polozeno : _polozeniPojasevi)
        {
            ...
            {
                polozeno.DodajTehniku(nazivTehnike, napomena.c_str());
                thread w(&KaratePolaznik::saljiMail, this, pojas, nazivTehnike, nazivTehnike.prosjecnaOcjenaZaTehniku(), polozeno.uspjehProsjekPolozeneTehnike());
                w.join();
                return true;
            }
        }
        Polaganje p(pojas);
        p.DodajTehniku(nazivTehnike, napomena.c_str());
        _polozeniPojasevi.push_back(p);
        thread w(&KaratePolaznik::saljiMail, this, pojas, nazivTehnike, nazivTehnike.prosjecnaOcjenaZaTehniku(), p.uspjehProsjekPolozeneTehnike());
        w.join();
        return true;
    }
